...
local customPropKeys = {
    ...
    key_member_nameflag = { name = "key_member_nameflag", default_value = "_e" },
}
...
toolMenu:AddItem("搞事情", "test", function(menuItem)
    -- for k,v in pairs(handler.items) do
    --     if v.type == "component" then
    --         local XMlItem = handler:GetItemDesc(v)
    --         fprint("type  "..v.name )
    --     end
    -- end
    fprint("Hello world")
...
function onPublish(handler)
    ...
    if (gen_lua == "true") then
        ...
        fprint("Handling gen lua code in plugin.发布成功")
        genCode(handler)
    end
end
...
function genCode(handler)
    ...
    local nameflag = get_project_custom_property_value(customPropKeys.key_member_nameflag.name);
    local nameflagLength = string.len(nameflag)
    fprint("nameflag  "..nameflag)
    fprint("nameflagLength  "..nameflagLength)
    ...
    for i = 0, classCnt - 1 do
        ...
        for j = 0, memberCnt - 1 do
            local memberInfo = members[j]
            if string.match(memberInfo.varName, nameflag, string.len(memberInfo.varName)-nameflagLength) then
                if matchIndex > 0 then
                    _classFieldInstatiation = _classFieldInstatiation .. "\n";
                    _Instatiation = _Instatiation .. "\n";
                end
                _classFieldInstatiation = _classFieldInstatiation .. "\t";
                _Instatiation = _Instatiation .. "\t";
                ...
                _Instatiation = _Instatiation..string.format('self.%s = nil',memberInfo.varName)
                matchIndex = matchIndex + 1
            end
        end
        _Instatiation = _Instatiation.."\n".."\t".."self.__ui = nil"
        _classTemplateTxt = string.gsub(_classTemplateTxt, "$Instantiation", _Instatiation);
    ...
    lua_path_root = get_project_custom_property_value(customPropKeys.key_lua_path_root.name);
